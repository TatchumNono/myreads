  ...
  const menus = () => {
    return (
      <Menu>
        ...
        <Menu.Item key='currentReading' onClick={(e) => changeShelf(book, e)}>
          Currently Reading
        </Menu.Item>
        <Menu.Item key='wantToRead' onClick={(e) => changeShelf(book, e)}>
          Want to read
        </Menu.Item>
        <Menu.Item key='read' onClick={(e) => changeShelf(book, e)}>
          Read
        </Menu.Item>
  ...
  const currentlyReading = () => {
    return (
      <Menu>
        ...
        <Menu.Item key='currentReading' icon={<CheckOutlined />}>
          Currently Reading
        </Menu.Item>
        <Menu.Item key='wantToRead' onClick={(e) => changeShelf(book, e)}>
          Want to read
  ...
  const wantToRead = () => {
    return (
      <Menu>
        ...
        <Menu.Item key='wantToRead' icon={<CheckOutlined />}>
          Want to read
        </Menu.Item>
        <Menu.Item key='read' onClick={(e) => changeShelf(book, e)}>
          Read
            ...
            ? result.map((book, index) => (
                <Col style={{ padding: " 32px 32px" }} span={6} key={index}>
                  <Card
                    ...
                    actions={[
                      <Dropdown
                        overlay={data.forE((item) => {
                          book.title === item.title ||
                          book.shelf === "wantToRead" ? (
                            <wantToRead item={item} />
                          ) : book.title === item.title ||
                            book.shelf === "currentReading" ? (
                            <currentlyReading />
                          ) : book.title === item.title ||
                            book.shelf === "read" ? (
                            <read />
